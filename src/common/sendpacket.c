 ...
 */
int sendpacket(sendpacket_t *sp, const u_char *data, size_t len, struct pcap_pkthdr *pkthdr)
{
    int retcode = 0, val;
...
#else
        retcode = (int)send(sp->handle.fd, (void *)data, len, 0);       // 向套接字中写入具体的数据
#endif
...
#endif
#if defined HAVE_PF_PACKET
            sp = sendpacket_open_pf(device, errbuf);        // 创建 PF_PACKET socket
...
sendpacket_open_pf(const char *device, char *errbuf)
{
    ...
    /* open our socket */
    // 创建 PF_PACKET socket，直接从 mac 层收发数据帧
    // ETH_P_ALL: 将所有类型的数据包都交给本socket（如果用该socket接收数据包，则将通过第三个参数过滤数据帧，然后将符合目标类型的数据帧传递给socket？？？）
 ...
 */
int sendpacket_get_dlt(sendpacket_t *sp)
{
    int dlt = DLT_EN10MB;
 ...
 */
void sendpacket_abort(sendpacket_t *sp)
{
    assert(sp);
